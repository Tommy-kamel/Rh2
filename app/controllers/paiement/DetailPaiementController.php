...
class DetailPaiementController
{
    public static function show($id_employe, $type, $mois_annee)
    {
        // Validation basique
        ...
        // Utilisation de Flight::render()
        Flight::render($view, ['data' => $data, 'title' => $type]);
    }

    public static function showIrsa()
    {
        $id_employe = $_GET['id_employe'];
         $type = "irsa";
         $mois_annee = $_GET['mois'];
        // Validation basique
        if (!in_array($type, ['irsa'])) {
            Flight::notFound();
            return;
        }
